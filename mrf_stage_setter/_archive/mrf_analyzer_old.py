...
LOG = logging.getLogger("src.mrf_analyzer")
...
def load_sql_file(filename: str) -> str:
    ...
    Args:
        filename: Name of the SQL file (e.g., "get_payloads_for_analysis.sql")
...
def analyze_json_structure(
    ...
    - For lists: yield, then process only the first item (if exists)
    ...
    Args:
        ...
        path: Full JSONPath to current location (e.g., "provider_references[0].provider_group_id")
    ...
    for key, value in data.items():
        ...
        if value_dtype == "scalar":
            ...
            # For lists: only process the first item
            if value and len(value) > 0:
                first_item = value[0]
                # Build path for list item: add [0] to current path
                list_item_path = f"{current_path}[0]"
                
                if isinstance(first_item, (dict, list)):
                    # Recursively process first item
                    if isinstance(first_item, dict):
                        ...
                        # Nested list: process first item of nested list
                        if first_item and len(first_item) > 0:
                            nested_first = first_item[0]
                            nested_list_path = f"{list_item_path}[0]"
...
def analyze_mrf_landing_records(
    ...
    using the first record's payload as representative of that record type's structure.
    ...
    try:
        ...
        LOG.info("Executing query to get payloads from mrf_landing table...")
        ...
        while True:
            ...
            for row in rows:
                ...
                # For arrays: payload is the first item (ingestion only extracts first item)
                ...
                batch.append((
                    ...
                    record_type,  # Path is just the key name for top-level
                ...
                # Analyze contents starting at level 1 (one level down from top-level key)
                for analysis_record in analyze_json_structure(
                    ...
                    if len(batch) >= batch_size:
                        insert_sql = load_sql_file("insert_analysis_record.sql")
                        execute_batch(
                            cursor,
                            insert_sql,
                            batch,
...
def create_mrf_analysis_table(connection_string: str, drop_if_exists: bool = False) -> None:
    ...
    try:
        ...
        from pathlib import Path
        ddl_path = Path(__file__).parent.parent / "sql" / "mrf_analysis.sql"
...
def get_urls_to_download(
    ...
    try:
        ...
        # Load SQL query from file
        query_sql = load_sql_file("get_urls_from_analysis.sql")
        cursor.execute(query_sql)
...
def analyze_urls_from_analysis(
    ...
    for fn, source_name, file_size, path, key, url in urls_to_download:
        try:
            ...
            LOG.debug("Processed URL for %s (path: %s): %d records inserted", fn, path, inserted)
        except Exception as exc:  # noqa: BLE001
            LOG.exception("Error processing URL for %s (path: %s): %s", fn, path, exc)
            continue
    
    LOG.info("Completed URL analysis: %d total records inserted from %d URLs", total_inserted, len(urls_to_download))
    return total_inserted
...
def run_shape_analysis(
    ...
    Returns:
        Tuple of (total_records, url_records) where:
    ...
    total_records = analyze_mrf_landing_records(
        connection_string=connection_string,
        batch_size=batch_size,
        fetch_batch_size=fetch_batch_size,
    )

    LOG.info("Successfully analyzed structures: %d analysis records inserted", total_records)
    ...
    LOG.info("Total analysis complete: %d records from JSON structures, %d records from URLs", 
             total_records, url_records)
    return total_records, url_records
