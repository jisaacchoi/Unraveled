#!/usr/bin/env python3
"""
Convert .json.gz files directly to Parquet/CSV files using PySpark with pre-generated schemas (TEST VERSION).
...
TEST VERSION: Uses pyspark_converter_test.py which filters to only NPI 1043268410 and skips group by.

Usage:
    python commands/05_convert_json_gz_to_parquet_test.py --config config.yaml
    
Note: 
    - Run 02_group_generate_schemas.py first to create the schema files and move files to analyzed_directory
...
from src.pyspark_converter_test import (
...
LOG = logging.getLogger("commands.json_gz_to_parquet_test")
...
def process_json_gz_file(
    ...
    TEST VERSION: Uses pyspark_converter_test functions which filter to only NPI 1043268410 and skip group by.
    ...
    try:
        ...
        if not schema:
            ...
            LOG.error("Make sure schemas have been generated using 04_group_generate_schemas.py")
        ...
        # Step 4: Process with existing conversion function (TEST VERSION: uses pyspark_converter_test)
...
def main() -> int:
    """Entrypoint for the json_gz_to_parquet_test command."""
    parser = argparse.ArgumentParser(description="Convert .json.gz files directly to Parquet/CSV files using PySpark (TEST VERSION)")
    ...
    log_file = get_log_file_path(config, "convert_json_gz_to_parquet_test")
    ...
    # Load provider_data from CSV file to filter NPIs (TEST VERSION: provider_npi_set will be ignored, only NPI 1043268410 will be used)
    provider_csv_path = json_gz_cfg.get("provider_data_csv_path", "D:/payer_mrf/filename_filtered_Illinois_institutions.csv")
    LOG.info("TEST VERSION: Loading provider_data from CSV file (will be ignored, only NPI 1043268410 will be used): %s", provider_csv_path)
    provider_npi_set = load_provider_data_npis(spark, provider_csv_path, npi_column="NPPES_NPI")
    if provider_npi_set is None:
        LOG.warning("Failed to load provider_data from CSV file. NPIs will be filtered to only 1043268410 (TEST VERSION).")
        provider_npi_set = None
    else:
        LOG.info("Successfully loaded provider_data from CSV file (TEST VERSION: will be ignored, only NPI 1043268410 will be used)")
    ...
    try:
        ...
        LOG.info("Processing %d .json.gz file(s)... (TEST VERSION: filtering to only NPI 1043268410, skipping group by)", len(files_to_process))
        if skip_if_schema_not_found:
